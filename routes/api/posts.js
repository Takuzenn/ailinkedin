...
router.post("/", urlencodedParser, async (req, res, next) => {
    ...
    if (!req.body.content) {
        console.log("no centent with request");
...
router.put("/:id/like", urlencodedParser, async (req, res, next) => {
...
router.post("/:id/retweet", urlencodedParser, async (req, res, next) => {
...
router.delete("/:id", urlencodedParser, async (req, res, next) => {
    ...
    if (post.retweetUsers !== undefined) {
        await Post.deleteMany({ retweetData: req.params.id})
        .catch(error => {
            console.log(error);
            res.sendStatus(400);
        });
